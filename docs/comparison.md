...
SDD Open does differently, so you can tell whether that difference matters for your work.

---

## The one-line difference

**Every spec-driven framework produces a spec. SDD Open makes the spec carry a command that
...
agent compares the implementation against written criteria and ticks a box. In SDD Open,
acceptance is a `verify_gate` block that `sdd/bin/verify-gate.sh` executes, returning one of six
exit codes that `/sdd-build` and `/sdd-release` are contractually required to honour.
...
cannot be expressed as a command — a visual judgment, a tone-of-voice call — SDD Open does not
...
| Capability | Spec-Kit | OpenSpec | BMAD | Kiro | Tessl | **SDD Open** |
|---|---|---|---|---|---|---|
...
| Editor/IDE independent | ✅ generated per-agent files | ✅ generated per-agent files | ✅ | ❌ (own IDE) | ⚠️ | ✅ **one source (`.agents/skills/`), adapters generated by `sdd-open.sh sync`; AGENTS.md managed block** |
| Adoption | ~129k★ | ~65k★ | ~52k★ | vendor | vendor | new |

**Read the last two rows together.** SDD Open is new and has no community; Spec-Kit has a large
...
- **OpenSpec's specs-as-diffs** solves a problem SDD Open has not solved: when many branches run
...
- **Spec-Kit's `/analyze`** checks bidirectional coverage — every requirement has a task, every
  task traces to a requirement. SDD Open has no equivalent yet, which is a real gap: its gate
...
published positions are cited because SDD Open is, in large part, an attempt to make their
...
*What SDD Open does with it:* the Verify Gate is that verification made mandatory and machine-read
...
[`ADVISOR_CONSULT.md`](../sdd/templates/fragments/ADVISOR_CONSULT.md): a fixed answer
...
*What SDD Open does with it:* those points are five non-negotiable guidelines in the build
...
"successfully implemented". The autonomy slider is explicit: `/sdd-build` runs in-context by
...
*What SDD Open does with it:* takes it as a constraint rather than a rebuttal. A framework that
...
document. SDD Open is for work where being wrong is expensive; Steinberger is right about
everything else.
